  ...
  if (!isset($_GET['c']) && !isset($_GET['s'])){
    for ($i=0; $i<count($product_names); $i++){
    	if (stristr($product_names[$i][2], $q)){
    		if ($hint == ""){
    			$hint = "<a href='preview-products-admin.php?pro=".$product_names[$i][2]."&i=".$i."'>".$product_names[$i][2]."</a>";
    		}
    		else{
    			$hint = $hint."<br><a href='preview-products-admin.php?pro=".$product_names[$i][2]."&i=".$i."'>".$product_names[$i][2]."</a>";
    		}
    		$max_count++;
    	}
    	if ($max_count>10){
