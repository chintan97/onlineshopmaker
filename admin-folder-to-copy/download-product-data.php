<?php
session_start();
$file = fopen("../product_data.json", 'r');

if (filesize("../product_data.json") == 0){
	echo "<script>alert('You have not entered any products till now!!!');</script>";
}
else{
	$file_read = fread($file, filesize("../product_data.json"));
	...
	if (count($json_array[$_SESSION['shop_name']]) == 0){
		...
		$list = array();
		$list_tags = array('category','subcategory','product name', 'product price', 'product stock', 'product threshold', 'product id', 'product brand', 'product size', 'product description', 'product gender', 'product offer price', 'product offer percentage', 'product color', 'warranty period', 'replacement period', 'product maximum quantity');
		...
		$file_csv = fopen('../product_data.csv', 'w');
