<?php
	session_start();
	$read_file = file_get_contents('user_folders/'.$_SESSION['username'].'/product_data.json');
	...
	if ($notification_count > 0){
		...
		if (!file_exists('user_folders/'.$_SESSION['username'].'/notifications.json') && count($notifications) > 0){
			$file = fopen('user_folders/'.$_SESSION['username'].'/notifications.json', 'w');
			$temp_count = 1;
			$data_to_write['root'] = [];
			...
			file_put_contents('user_folders/'.$_SESSION['username'].'/notifications.json', json_encode($data_to_write));
			fclose($file);
			echo $notification_count;
		}
		else {
			$file_noti = fopen('user_folders/'.$_SESSION['username'].'/notifications.json', 'a+');
			$file_data_noti = fread($file_noti, filesize('user_folders/'.$_SESSION['username'].'/notifications.json'));
		...
		if (file_exists('user_folders/'.$_SESSION['username'].'/notifications.json')){
			$data['root'] = [];
			file_put_contents('user_folders/'.$_SESSION['username'].'/notifications.json', json_encode($data));
		}
		echo "0";
	}
?>